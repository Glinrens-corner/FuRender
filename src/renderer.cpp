...
namespace furender {
  namespace detail {
    void Renderer::render_internal(explicit_key_t key,
      ...
      std::optional<WidgetInstanceData *> old_data;
      ...
      // if the old instance is still current, return the value of the last
      // execution.
      if (old_data.has_value()) {
        if (this->subinstance_is_current(
              *widget_ptr, instance_id, *old_data.value())) {
          return;
        }
      }
      ...
      RenderVisitor visitor(
        this->state_, this->render_tree_, collecting_context);
    ...
    std::optional<WidgetInstanceData *> Renderer::get_old_instance_data(
      explicit_key_t key, widget_instance_id_t parent_id) const {
      auto parent_optr =
        this->render_tree_->get_widget_instance_data(parent_id);
      if (not parent_optr.has_value()) return {};
      WidgetInstanceData *parent_ptr = parent_optr.value();

      auto it =
        std::find_if(parent_ptr->children.begin(), parent_ptr->children.end(),
          [key](const std::pair<explicit_key_t, widget_instance_id_t>& pair) {
            return pair.first == key;
          });
      if (it != parent_ptr->children.end()) {
        auto child_optr =
          this->render_tree_->get_widget_instance_data(it->second);
    ...
    std::optional<WidgetInstanceData *> Renderer::get_root_instance_data() {
      return this->render_tree_->get_root_instance_data();
    }

    bool Renderer::subinstance_is_current(const BaseWidget& widget,
      widget_instance_id_t instance_id,
      const WidgetInstanceData& data) const {
      return (widget.get_widget_id() == data.widget->get_widget_id() and
              (not this->render_tree_->has_to_be_updated(instance_id)));
    }
