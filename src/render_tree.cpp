...
namespace furender{
  ...
  widget_instance_id_t RenderTree::get_next_widget_to_update() const{
    ...
    {
	widget_instance_id_t parent = this->render_tree_.find(*instance_to_update_it)->second.parent_id;
  ...
  void RenderTree::render_all(){
    ...
    if(not this->root_instance_.has_value()){
      assert(this->widget_instances_to_update_.empty() && "there are no widgets that could already be needed to update");
  ...
  void RenderTree::render_render_queue_entry(RenderQueueEntry &entry){
    ...
    detail::Renderer(this, this->state_).render_internal(entry.key, entry.parent_id,std::move(entry.widget), &collecting_context);
  }



  void RenderTree::rerender_instance(widget_instance_id_t new_instance_id, CollectingContext* collecting_context){
  ...
  std::optional<WidgetInstanceData*> RenderTree::get_widget_instance_data(widget_instance_id_t id){
  ...
  void RenderTree::merge_collecting_context(CollectingContext& root_collecting_context){
    ...
    while(not collecting_context_stack.empty()){
	  ...
	  && (not collecting_context_stack.back()->new_instance_data.value().subcontexts.empty() )){

        collecting_context_stack
	  .push_back(collecting_context_stack.back()
	...
	// the collecting_context will be completely handled. therfore we can already erase it.
	collecting_context_stack.pop_back();

        // if there is no new instance data, the widget doesn't have to be updated.
	if (! current_context.new_instance_data.has_value()){
	  ...
	  if(collecting_context_stack.size() > 0  ){
	    ...
	    // using the fact, that we always push a pointer to the last subcontext onto the stack. therefore the last subcontext of parent ist the current context
            auto& [key, collecting_context_uptr /*unused */ ]= parent_context.new_instance_data.value().subcontexts.back();
	    assert(collecting_context_uptr.get()== &current_context && "the accessed subcontext of the parent must be the same as the current context");
	  ...
	  //collection context is soon to be destroyed, we can move the children out.
	  instance_data.children = std::move(current_context.new_instance_data.value().children);
  ...
  void RenderTree::remove_subscriptions_from_state(const WidgetInstanceData& instance_data){
