...
namespace furender {
  ...
  struct is_data_event_handling<DataEvent<data_t_>, data_t_> : std::true_type {
  };
  ...
  class StateContext {
    ...
    public:
      StateContext(TypedStateSlice<state_t_> *slice): slice_(slice) {
        this->is_updated_ = this->get_initial_is_updated_value();
      };

      bool is_updated() { return this->is_updated_; }
  ...
  class StateDispatchVisitor {
    ...
    public:
      StateDispatchVisitor(EventEnvelope event_envelope)
        : event_envelope_(std::move(event_envelope)){};
  ...
  class TypedStateSlice : public StateSlice {
    ...
    public:
      explicit TypedStateSlice(state_t state)
        : StateSlice(),
          state_(std::move(state)) {}

      TypedStateSlice() {}

      virtual state_t *get_state_ptr() { return &this->state_; }
      ...
      const state_t_& state() const { return this->state_; }

    private:
      static_assert(std::is_default_constructible_v<state_t_>,
  ...
  class ReducingStateSlice<state_t_,
    template_list<template_list<event_ts_...>,
    ...
    public:
      ReducingStateSlice(
        state_t_ initial_value, std::tuple<callable_ts_...> reducers)
        : TypedStateSlice<state_t_>(std::move(initial_value)),
          reducers_(std::move(reducers)){};

      bool accept_dispatch_visitor(
        StateDispatchVisitor& visitor) override final {
      ...
      UpdateType update_type() override final { return update_type_; }
    ...
    private:
      template <std::size_t i>
        ...
        i<sizeof...(callable_ts_), void>::type try_next_handler(
          StateContext<state_t_>& context, const BaseEvent& event) {
      ...
      typename std::enable_if<i == sizeof...(callable_ts_), void>::type
        try_next_handler(StateContext<state_t_>&, const BaseEvent&){};

      template <std::size_t i>
      bool try_nth_handler(StateContext<state_t_>&, const BaseEvent& event) {
        using event_t =
          typename template_list_element<i, template_list<event_ts_...>>::type;
        using invocation_t = typename template_list_element<i,
          template_list<invocation_ts_...>>::type;
        const event_t *event_ptr = dynamic_cast<const event_t *>(&event);
        if (event_ptr != nullptr) {
          if (this->handle_data_event<i, event_t, invocation_t>(*event_ptr))
            return true;
          if (handle_signal_event<i, event_t, invocation_t>(*event_ptr))
            return true;
          if (handle_common_event<i, event_t, invocation_t>(*event_ptr))
            return true;
        }
        return false;
      };
  ...
  namespace detail {
    ...
    template <class state_t_,
      ...
      class... reducer_ts_,
      UpdateType update_type_>
    class ReducingStateSliceBuilder<state_t_,
      template_list<template_list<event_ts_...>,
        template_list<invocation_ts_...>,
      ...
      public:
        ReducingStateSliceBuilder(
          state_t_&& initial_value, std::tuple<reducer_ts_...>&& reducers)
        ...
        auto with_event_reducer(reducer_t reducer) {
          ...
          using namespace std;
          using new_builder_t = ReducingStateSliceBuilder<state_t_,
            template_list<template_list<event_ts_..., event_t>,
          ...
          return new_builder_t(std::move(this->initial_value_),
            tuple_cat(
              std::move(this->reducers_), std::make_tuple(std::move(reducer))));
        }
        ...
        auto with_data_reducer(reducer_t reducer) {
          using namespace std;
          using new_builder_t = ReducingStateSliceBuilder<state_t_,
            template_list<template_list<event_ts_..., DataEvent<data_t>>,
          ...
          return new_builder_t(std::move(this->initial_value_),
            tuple_cat(
              std::move(this->reducers_), make_tuple(std::move(reducer))));
        }
        ...
        auto with_signal_reducer(reducer_t reducer) {
          using namespace std;
          using new_builder_t = ReducingStateSliceBuilder<state_t_,
            template_list<template_list<event_ts_..., SignalEvent>,
        ...
        auto make_unique_ptr() {
          auto u_ptr = std::unique_ptr<ReducingStateSlice<state_t_,
            template_list<template_list<event_ts_...>,
              template_list<invocation_ts_...>,
              template_list<reducer_ts_...>>>>(new ReducingStateSlice<state_t_,
            template_list<template_list<event_ts_...>,
              template_list<invocation_ts_...>, template_list<reducer_ts_...>>>(
  ...
  auto create_state_slice(state_t initial_value) {
    ...
    return detail::ReducingStateSliceBuilder<state_t,
  ...
  class State {
    ...
    public:
      ...
      const typename selector_t::selected_t *get_state_ptr(
        const selector_t& selector) {
        using state_t = typename selector_t::selected_t;
        StateSlice *slice = this->get_state_slice(selector.path());
        TypedStateSlice<state_t> *slice_typed =
          dynamic_cast<TypedStateSlice<state_t> *>(slice);
        if (slice_typed) {
      ...
      void set_state_slice(Addressor<state_t, event_ts...>& addressor,
        ...
        if (!this->slices_
               .insert({
                 addressor.path(), SliceData{std::move(slice), {}}
        })
               .second) {
      ...
      void set_state_slice(
        std::string path, std::unique_ptr<StateSlice> slice) {
        if (!this->slices_
               .insert({std::move(path), SliceData{std::move(slice)}})
               .second) {
          throw "Error problem accepting a new state slice";
        };
      }

      void accept_subscription(
        const std::string& path, widget_instance_id_t instance_id) {
      ...
      void remove_subscription(
        ...
        if (it != this->slices_.end()) {
          ...
          auto current_it =
            std::find(subscriptions.begin(), subscriptions.end(), instance_id);
