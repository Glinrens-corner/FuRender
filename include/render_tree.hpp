...
namespace furender {
  ...
  class InstanceIdGenerator {
    ...
    public:
      InstanceIdGenerator(widget_instance_id_t::value_t id)
        : current_instance_value_(id){};
  ...
  class ClientIdGenerator {
    ...
    public:
      ClientIdGenerator(client_instance_id_t::value_t id)
        : current_instance_value_(id){};
  ...
  class RenderTree {
    ...
    public:
       ...
       *
       */
      WidgetInstanceData *insert_instance(
        widget_instance_id_t, WidgetInstanceData&&);

      /** @brief get the pointer to the data of an instance.
       *
       * if the return  has a value, it is dereferencable.
       */
      std::optional<WidgetInstanceData *> get_widget_instance_data(
    ...
    public:
      ...
      /** @brief announce that a property (e.g. StateSlice) a widget instance
       * depends upon has changed
       *
       */
      ...
      State& get_state() { return *this->state_; }

      /**@brief get the data of the root widget.
       *
       */
      std::optional<WidgetInstanceData *> get_root_instance_data() {
        if (this->root_instance_.has_value()) {
          return this->get_widget_instance_data(this->root_instance_.value());
        } else {
          return {};
        }
      }
...
#ifndef NDEBUG
      std::optional<widget_instance_id_t> debug_get_root_instance() const {
...
#ifndef NDEBUG
      const std::deque<RenderQueueEntry>& debug_get_render_queue() const {
        return this->render_queue_;
      }
#endif

#ifndef NDEBUG
      const std::unordered_set<widget_instance_id_t>&
        debug_get_widget_instances_to_update() const {
    ...
    private:
       ...
       */
      void insert_or_update_instance(
        WidgetInstanceData&&, std::optional<WidgetInstanceData *>);
