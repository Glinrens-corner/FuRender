...
namespace furender {
  ...
  namespace detail {
    ...
    struct sequence_generator
      : sequence_generator<first - 1, first - 1, numbers...> {};
    ...
    template <class return_t,
      ...
      class tuple_t,
      int... numbers>
    return_t call_render(widget_t& widget,
      context_t& context,
      const tuple_t& tuple,
     ...
     * @param args_tuple tuple of the arguments later to be supplied to the
     * render method.
    ...
    void fill_args_tuple(args_tuple_t& args_tuple,
      const selector_tuple_t& selector_tuple,
      State& state) {
      if constexpr (i < std::tuple_size_v<args_tuple_t>) {
        // TODO: communication via nullptr is forbidden
        const std::tuple_element_t<i, args_tuple_t> *p =
          state.get_state_ptr(std::get<i>(selector_tuple));
        if (p) {
  ...
  class RenderVisitor {
    ...
    public:
      RenderVisitor(
        State *state, RenderTree *tree, CollectingContext *collecting_context)
        : state_(state),
          tree_(tree),
          collecting_context_(collecting_context) {}

      /** @brief set up all arguments for the widget.render function and call
       * it.
       *
       */
      ...
      void render(widget_t& widget) {
        ...
        Context<widget_type> context = Context<widget_type>(
          this->collecting_context_, this->tree_, this->state_);
        ...
        detail::fill_args_tuple<0>(
          args_tuple, widget.selectors(), *this->state_);

        // apply the arguments to the render method
        using sequence_t = typename detail::sequence_generator<
          std::tuple_size_v<args_tuple_t>>::sequence_t;
        this->collecting_context_->new_instance_data.value().return_value =
          std::make_unique<ValueHolder<return_t>>(detail::call_render<return_t>(
            widget, context, args_tuple, sequence_t())
        ...
        static_assert(!std::is_same_v<return_t, void>,
          "a widgets return type may not be void");
        static_assert(std::is_default_constructible_v<return_t>,
          "a widgets return type must be default constructible");
