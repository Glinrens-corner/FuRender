...
TEST_CASE("simple render ()", "[render][State][Widget][RenderTree]") {
  ...
  auto widget =
    create_widget_with_selectors<furender::WidgetType::Application>()
...
TEST_CASE("simple render of a widget referencing state",
  "[render][State][Widget][RenderTree][StateSlice]") {
  using namespace furender;
  int spy = 0;
  auto slice = create_state_slice<int>(4)
                 .with_data_reducer<int>(
                   [](furender::StateContext<int>& context,
                     const int& new_int) { context.update_state(new_int); })
                 .make_unique_ptr();
  ...
  auto widget =
    create_widget_with_selectors<furender::WidgetType::Application>(
      addressor.create_selector())
      .with_render_function(
        [&spy](Context<WidgetType::Application>&, const int new_value) {
          spy = new_value;
          return None::none;
        })
      .make_shared();
...
TEST_CASE("render without update", "[render][State][Widget][RenderTree]") {
  ...
  auto client_widget =
    create_widget_with_selectors<WidgetType::Client>()
  ...
  auto widget =
    create_widget_with_selectors<WidgetType::Application>()
      .with_render_function(
        [&spy1, client_widget](Context<WidgetType::Application>& context) {
          None n = context.immediate_render(
            1, static_cast<std::shared_ptr<Widget<WidgetType::Client, None>>>(
                 client_widget));
          (void)n;
          spy1 += 1;
          return None::none;
...
TEST_CASE(
  "render with update", "[render][State][Widget][RenderTree][StateSlice]") {
  using namespace furender;
  int spy1 = 0;
  int spy2 = 100;
  ...
  auto app_widget =
    create_widget_with_selectors<WidgetType::Application>(
      ...
      .with_render_function(
        [&spy1, client_widget](
          Context<WidgetType::Application>& context, const int& other_value) {
          None n = context.immediate_render(
            1, static_cast<std::shared_ptr<Widget<WidgetType::Client, None>>>(
                 client_widget));
          (void)n;
  ...
  CHECK(spy2 == 101);
  SECTION(
    "if the state_slice hasn't been updated, the next render doesn't render anything") {
    tree.render_all();
    CHECK(spy1 == 1);
    CHECK(spy2 == 101);
  };
  ...
  SECTION(
    "if the state_slice has updated, the next render rerenders the affected widget") {
